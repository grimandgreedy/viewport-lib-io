...
use super::blendshape::ExtractedBlendShape;

/// One mesh model's vertex arrays as read off its geometry: triangulated, with
/// every UV layer still a candidate and no material split applied.
struct MeshVertices {
    positions: Vec<[f32; 3]>,
    normals: Vec<[f32; 3]>,
    /// The control point each emitted vertex came from. Skin influences and
    /// blend-shape deltas are stored per control point, so both expand to one
    /// value per emitted vertex through this.
    vertex_cp_index: Vec<u32>,
    control_point_count: usize,
    uv_candidates: Vec<Vec<[f32; 2]>>,
    /// Each candidate's layer name, parallel to `uv_candidates`. A texture names
    /// the layer it samples by name, and a layer that turns out not to be a UV
    /// set is emitted as a named attribute, so both ends need it.
    uv_layer_names: Vec<String>,
    material_indices_per_vert: Option<Vec<usize>>,
}

/// What became of a mesh's UV layers: the two sets it emits, with V flipped, and
/// whatever is left over as named per-vertex attributes.
struct UvSets {
    uvs: Option<Vec<[f32; 2]>>,
    uvs1: Option<Vec<[f32; 2]>>,
    leftover_layers: Vec<(String, Vec<[f32; 3]>)>,
}

/// One mesh model decoded, ready to be emitted as a single mesh or split one way
/// per material.
struct DecodedModel<'a> {
    name: String,
    transform: glam::Mat4,
    materials: &'a [usize],
    positions: Vec<[f32; 3]>,
    ...
    material_indices_per_vert: Option<Vec<usize>>,
    uv: UvSets,
    skin_weights: Option<SkinWeights>,
    skeleton_index: Option<usize>,
    morph_targets: Vec<(String, Vec<[f32; 3]>)>,
}
...
pub(super) fn build_fbx_scene(
    ...
    // Diagnostics for the UV pick. Read once rather than once per mesh.
    ...
    for object in objects_in_stable_order(&document) {
        if let TypedObjectHandle::Model(TypedModelHandle::Mesh(mesh_model)) = object.get_typed() {
            ...
            let model_materials = resolve_model_materials(
                &mesh_model,
                parent_dir,
                &mut materials,
                &mut material_map,
            ...
            let Ok(geometry) = mesh_model.geometry() else {
                continue;
            };
            let Some(vertices) = read_mesh_vertices(&geometry) else {
                continue;
            };
            let MeshVertices {
                positions,
            ...
            let (skin_weights, skeleton_index) = match extract_skin(
                ...
                Some(skin) => {
                    let weights = expand_skin(&skin.cp_influences, &vertex_cp_index);
                    let skeleton_index = skeletons.len();
                    skeletons.push(skin.skeleton);
                    (Some(weights), Some(skeleton_index))
                }
                None => (None, None),
            };

            // Blend shapes: harvest per-control-point deltas, then expand to one
            // delta per emitted vertex through the same control-point map the
            // skin path uses, so a target lines up with `positions`. Split across
            // sub-meshes below exactly like positions / skin.
            let blend_shapes = extract_blend_shapes(&geometry, control_point_count);
            let morph_targets = expand_blend_shapes(&blend_shapes, &vertex_cp_index);

            let uv = choose_uv_sets(
                uv_candidates,
                &uv_layer_names,
                &model_name,
                model_materials.len(),
    ...
    apply_uv_set_names(&mut materials, pending_uv_sets, &uv_set_indices);
...
/// Convert every material this model names, reusing one already converted for an
/// earlier model. A textured slot that names a UV set records the name for
/// [`apply_uv_set_names`], which resolves it once every mesh has decided which of
/// its layers is UV0.
fn resolve_model_materials(
    mesh_model: &fbxcel_dom::v7400::object::model::MeshHandle<'_>,
    parent_dir: &Path,
    materials: &mut Vec<IoMaterial>,
    material_map: &mut HashMap<i64, usize>,
    pending_uv_sets: &mut Vec<(usize, MaterialTextureSlot, String)>,
) -> Vec<usize> {
...
/// Read one geometry's triangulated vertex arrays: positions, normals, every
/// decodable UV layer, and the per-vertex material assignment. `None` when the
/// geometry carries nothing this loader can emit.
fn read_mesh_vertices(
                        ...
                        if ok && uvs.len() == positions.len() {
                            ...
                            uv_layer_names.push(
                                uv_handle
                                    .name()
                                    .map(std::borrow::ToOwned::to_owned)
                                    .unwrap_or_else(|_| format!("uv{}", uv_candidates.len() - 1)),
                            );
                        }
                    }
                }
    ...
    Some(MeshVertices {
        ...
        normals,
        vertex_cp_index,
        control_point_count: (max_cp_index as usize).saturating_add(1),
        uv_candidates,
        uv_layer_names,
        material_indices_per_vert,
    })
...
/// Expand per-control-point skin influences to one entry per emitted vertex,
/// keeping the four heaviest the neutral type carries.
fn expand_skin(cp_influences: &[Vec<(u8, f32)>], vertex_cp_index: &[u32]) -> SkinWeights {
    let mut joint_indices: Vec<[u8; 4]> = Vec::with_capacity(vertex_cp_index.len());
    let mut joint_weights: Vec<[f32; 4]> = Vec::with_capacity(vertex_cp_index.len());
    for &cp in vertex_cp_index {
        let influences = cp_influences
            .get(cp as usize)
            .map(|v| v.as_slice())
            .unwrap_or(&[]);
        let mut idx = [0u8; 4];
...
/// Expand each blend shape's per-control-point deltas to one delta per emitted
/// vertex, through the same map [`expand_skin`] uses, so a target lines up with
/// `positions`.
fn expand_blend_shapes(
    blend_shapes: &[ExtractedBlendShape],
    vertex_cp_index: &[u32],
) -> Vec<(String, Vec<[f32; 3]>)> {
    blend_shapes
        .iter()
        .map(|bs| {
...
/// A UV layer's bounding extent as `(u_min, u_max, v_min, v_max)`, for the
/// `VIEWPORT_FBX_LOG_UV` diagnostics.
fn uv_extent(uvs: &[[f32; 2]]) -> (f32, f32, f32, f32) {
...
/// Decide which of a mesh's UV layers become UV0 and UV1, flip V on them for the
/// renderer's convention, and hand back whatever is left as named per-vertex
/// attributes. Which layer name ended up as which set is recorded in
/// `uv_set_indices`, for the textures that name one.
fn choose_uv_sets(
    uv_candidates: Vec<Vec<[f32; 2]>>,
    uv_layer_names: &[String],
    model_name: &str,
    material_count: usize,
    uv_set_indices: &mut HashMap<String, u32>,
    log_uv: bool,
) -> UvSets {
    ...
    if log_uv && !uv_candidates.is_empty() {
        ...
        for (i, uvs) in uv_candidates.iter().enumerate() {
            let (umin, umax, vmin, vmax) = uv_extent(uvs);
            let max_abs = uvs
                .iter()
...
/// Emit a decoded model: one mesh per material where the geometry assigns
/// materials per polygon, otherwise a single mesh. The sub-mesh path slices every
/// per-vertex array the same way, so positions, UVs, skin weights, and morph
/// deltas stay in step.
fn push_model_meshes(meshes: &mut Vec<IoMesh>, model: DecodedModel<'_>, log_uv: bool) {
    let DecodedModel {
        name: model_name,
        transform: node_transform,
        materials: model_materials,
        ...
        skin_weights: skin_per_vertex,
        skeleton_index,
        morph_targets: full_morph_targets,
    } = model;
            ...
            for (local_material_index, vertex_indices) in groups.into_iter().enumerate() {
                ...
                if log_uv {
                    if let Some(ref su) = sub_uvs {
                        let (umin, umax, vmin, vmax) = uv_extent(su);
                        eprintln!(
                ...
                let sub_morphs: Vec<MorphTarget> = full_morph_targets
                    ...
                    .map(|(name, deltas)| MorphTarget {
                        name: name.clone(),
                        position_deltas: vertex_indices.iter().map(|&i| deltas[i]).collect(),
                        normal_deltas: None,
                        tangent_deltas: None,
                    })
                    .collect();
    ...
    if log_uv {
        if let Some(ref u) = mesh_data.uvs {
            let (umin, umax, vmin, vmax) = uv_extent(u);
            eprintln!(
...
/// emitted. A name that matches nothing leaves the slot on UV0: the file named a
/// layer this decode did not carry, and UV0 is the set every consumer has.
fn apply_uv_set_names(
    materials: &mut [IoMaterial],
    pending_uv_sets: Vec<(usize, MaterialTextureSlot, String)>,
    uv_set_indices: &HashMap<String, u32>,
) {
